<template>
    <div class="container">
        <div class="settings">
            ...
            <button @click="sortPages" class="settings__sort">Cортировать</button>
        </div>
        <table class="table">
            <thead>
            <tr>
            ...
            <tr v-for="item in this.data" :key="item._id">
                <td data-label="Название">
                    ...
                    <router-link :to="'/library/' + item._id">Узнать подробнее</router-link>
                </td>
                <td data-label="Адрес">{{item.data.general.address.fullAddress}}</td>
                <td data-label="Часы работы">
                    <div>
                        <p v-if="item.data.general.workingSchedule[1]">пн-пт:
                            {{item.data.general.workingSchedule[1].from.substr(0, 5)}} -
                            {{item.data.general.workingSchedule[1].to.substr(0, 5)}}</p>
                        <p v-if="item.data.general.workingSchedule[6]">сб:
                            {{item.data.general.workingSchedule[6].from.substr(0, 5)}}
                            - {{item.data.general.workingSchedule[6].to.substr(0, 5)}}</p>
                    </div>
                </td>
            </tr>
            </tbody>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            sortPages() {
                axios.get(`${this.confs.hostUrl}_sort=data.general.locale.name&_order=asc&_page=1`)
                    .then(resp => {
                        this.data = resp.data;
            ...
            resetFilters() {
                axios.get(`${this.confs.hostUrl}_page=1`)
                    .then(resp => {
                        this.data = resp.data;
                        this.header = resp.headers.link;
                        this.page = 1;
